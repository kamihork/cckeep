...
- `stuckLimit`: `reconnecting` の表示が何回続いたら固まったとみなすか。`interval` を掛けた値が実時間になるので、既定では35分です。Claude Code 自身が自力で再接続を続ける約30分より必ず長くしてください。短いと、戻ってくるはずの接続を cckeep が切ってしまいます。**0.6.2 より前から使っている場合**、`config.json` に `8` が書かれていると2分のままなので、上げてください
