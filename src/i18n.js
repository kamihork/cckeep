const en = {
  ...
  limitWait: 'waiting out a usage limit',
  limitGaveUp: 'usage limit outlasted the retries — pick it up by hand',
  noPrompt: 'limitResumePrompt is empty — nothing to resume with',
  switched: (m) => `usage limit — switched to ${m}`,
  restored: (m) => `switched back to ${m}`,
  resumed: 'resumed the work',
  wouldSwitch: (m) => `would switch to ${m}`,
  wouldRestore: (m) => `would switch back to ${m}`,
  wouldResume: 'would resume the work',
...
const ja = {
  ...
  limitWait: '使用量の上限が解除されるのを待っています',
  limitGaveUp: '上限が長引いて打ち切りました(手動で再開してください)',
  noPrompt: 'limitResumePrompt が空のため再開できません',
  switched: (m) => `使用量の上限のため ${m} に切り替えました`,
  restored: (m) => `${m} に戻しました`,
  resumed: '作業を再開しました',
  wouldSwitch: (m) => `${m} に切り替えます`,
  wouldRestore: (m) => `${m} に戻します`,
  wouldResume: '作業を再開します',
